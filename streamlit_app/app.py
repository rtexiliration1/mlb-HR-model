...
APP_VERSION = "v31_1_curated_primary_board_display_plus_parlay_builder"
...
DISPLAY_TABS: tuple[DisplayTab, ...] = (
    ...
    # 6. General parlay-builder market board
    DisplayTab(
        label="Parlay Builder",
        aliases=(
            "Parlay Builder Market Boards",
            "Parlay Builder Market Board",
    ...
    # 12. New champion-picks card.  Prefer the newest multi-lane card first,
...
def render_tab(spec: DisplayTab, rows: list[dict[str, Any]], key_scope: str = "tab"):
    ...
    search = st.text_input(
        "Search this tab",
        key=f"search_v31_1_{key_scope}_{spec.label}",
...
def main():
    ...
    st.success("Portal patch loaded: v31.1 — curated primary boards + Parlay Builder + Champion Picks Card")
    st.caption("Primary display: All HR Rankings, game coverage, top 3 per game, best 2-leg boards, Parlay Builder, top Hits/RBI/K/Walk/ML, and the Champion Picks Card. Audit/register/archive sheets remain blocked.")
    ...
    selected_label = st.selectbox(
        ...
        labels,
        index=latest_idx,
        key="published_run_selector_v31_1",
    )
