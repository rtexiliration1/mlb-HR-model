...
APP_VERSION = "v29_force_dynamic_market_board_nav"
...
def main():
    ...
    st.success("Portal patch loaded: v29 force dynamic market-board navigation — Top 10/Top 5 boards enabled; Core HR Top 30 blocked")
    ...
    selected_label = st.selectbox(
        ...
        labels,
        index=latest_idx,
        key="published_run_selector_v29",
    )
    ...
    with st.expander("Display/upload guardrail summary", expanded=False):
        ...
        st.caption("Blocked by default: Core HR Top 30, Daily Prediction Register, register/carryforward/source-graph/manifest/validation sheets. The only displayed audit-style sheet is Recap-Driven Market Board Audit.")

    # Build the top navigation from allowed sheets that actually returned rows.
    # This prevents stale/hardcoded tabs such as Core HR Top 30 from appearing
    # and makes newly imported market-board sheets accessible immediately.
    visible_specs = [spec for spec in DISPLAY_TABS if rows_by_tab.get(spec.label)]
    if not visible_specs:
        visible_specs = list(DISPLAY_TABS)

    hidden_specs = [spec.label for spec in DISPLAY_TABS if not rows_by_tab.get(spec.label)]
    if hidden_specs:
        st.caption(
            "No rows loaded for these allowed tabs on the selected run: "
            + ", ".join(hidden_specs)
        )

    # Extra access control for wide displays: Streamlit tabs can overflow
    # horizontally. The selector guarantees every visible board is reachable
    # even if the tab row scrolls off-screen.
    selected_board = st.selectbox(
        "Quick access board",
        [spec.label for spec in visible_specs],
        index=0,
        key="quick_access_board_v29",
    )

    tabs = st.tabs([spec.label for spec in visible_specs])
    for spec, tab in zip(visible_specs, tabs):
        with tab:
            if spec.label == selected_board:
                st.caption("Selected in quick access.")
